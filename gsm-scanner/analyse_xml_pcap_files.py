...
def parseFiles(root, files):
    ...
    for fileName in files:
        ...
        db = sqlite3.connect(sqlite_file)
        if (len(CELLS) == 0):
            db.execute(sqlite_insert_query, (timeFromScan, num(channel), 0, 0,
             0, "00", EVERYTHING[timeFromScan]["GPS"]["lat"],
             EVERYTHING[timeFromScan]["GPS"]["lon"], EVERYTHING[timeFromScan]["GPS"]["time"]))
        
        else: # If there multiple cells extract only the first one since they are all the equal
            db.execute(sqlite_insert_query, (timeFromScan, num(channel), CELLS[0]["CELL"], CELLS[0]["LAC"],
             CELLS[0]["MCC"], CELLS[0]["MNC"], EVERYTHING[timeFromScan]["GPS"]["lat"],
             EVERYTHING[timeFromScan]["GPS"]["lon"], EVERYTHING[timeFromScan]["GPS"]["time"]))
